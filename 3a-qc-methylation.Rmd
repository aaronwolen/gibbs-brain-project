...
# Currently ggaffy_scatgrid cannot deal with NA values. When this is fixed it
# won't be necessary to filter by na.probes.
na.probes <- apply(exprs(eset.meth), 1, function(x) sum(is.na(x)))

meth.sub <- eset.meth[na.probes == 0, eset.meth$tissue == "CRBLM"]
...
meth.sub <- eset.meth[na.probes == 0, eset.meth$tissue == "PONS"]
sampleNames(meth.sub) <- meth.sub$individual
ggaffy_scatgrid(meth.sub)
```
...
meth.sub <- eset.meth[na.probes == 0, eset.meth$tissue == "FCTX"]
sampleNames(meth.sub) <- meth.sub$individual
ggaffy_scatgrid(meth.sub)
```
...
meth.sub <- eset.meth[na.probes == 0, eset.meth$tissue == "TCTX"]
sampleNames(meth.sub) <- meth.sub$individual
ggaffy_scatgrid(meth.sub)
```