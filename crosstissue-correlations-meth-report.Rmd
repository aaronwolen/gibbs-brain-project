...
```{r r-tissue-pairs, fig.height=5.5}
r.mat <- dcast(data = cors.df, 
  id + chr + cpg.island + distance.to.tss ~ tissue.pair, value.var = "r")


plot_matrix(r.mat, facet.vars = levels(cors.df$tissue.pair), density = TRUE) +
  xlab("Cross-tissue correleation") + ylab("Cross-tissue correlation")
```
...
for(p in probes) {
  ...
  probe.plots[[p]] <- plot_matrix(p.df, mapping = aes(color = gender),
