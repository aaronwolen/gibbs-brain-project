...
opts_chunk$set(fig.path = file.path(fig.dir, fig.prefix),
  ...
  cache=TRUE, autodep=TRUE, fig.width=5, fig.height=4, dpi=150)
```
...
```{r count-correlations, fig.width=5, fig.height=3}
...
```{r count-correlations-tables, results='asis'}
tissues <- c("CRBLM" = "Cerebellum", "FCTX" = "Frontal cortex", 
   "TCTX" = "Temporal cortex", "PONS" = "Pons")

for(t in seq_along(tissues)) {
  cat("\n####", tissues[t], "\n")
  cur.t <- names(tissues)[t]
  print_xtable(subset(mir.counts, tissue == cur.t, select = -tissue)[1:10,])
}
```
...
```{r count-correlations-direction, fig.width=5, fig.height=3}
...
### Scatter plots
Scatter plots for the 50 most significant microRNA/mRNA correlations are provided [here](https://github.com/aaronwolen/gibbs-brain-project/blob/master/figures/crossdata-correlations-micro-exp/cors-micro-exp-top-probes.pdf).

```{r sig-probes-scatterplots, include=FALSE}
