...
The analysis was performed independently for each tissue. Subsets of the expression and methylation eSet objects were created to include only samples from individuals present in both datasets. Methylation levels were adjusted for age and sample tissue bank by fitting a linear model with these variables and using the residuals in the correlation analysis. 
